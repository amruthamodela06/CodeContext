...
- **Current slice**: Slice 6 complete — hybrid retrieval (BM25 + vector, RRF-fused) + optional cross-encoder rerank, all four modes A/B-switchable via `RETRIEVAL_MODE` env. Postgres FTS via generated `tsvector` columns on chunks / commits / PRs / issues (chunks use Python-managed intermediates for camelCase / snake_case identifier splitting, weighted A/B/D). New `Retriever` protocol (third instance of the provider pattern after `Embedder` / `LLMProvider`) — retrievers return pointers, hydration stays in Slice 5's orchestrator. `RerankedRetriever` uses `BAAI/bge-reranker-base` via `sentence-transformers.CrossEncoder` (~280 MB in HF cache, ~1-2 s per query on CPU, lazy-loaded). Debug surface: `score_breakdown` on every retrieved entity + `stage_timings_ms` on the trace, both rendered in the frontend debug toggle. Slices 1–5 still good. Slice 7 next = evaluation harness (recall@5/10, citation accuracy, LLM-as-judge, per-mode ablation). See ADR 0014.
